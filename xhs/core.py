...
class XhsClient:
    ...
    def save_user_all_notes(self, user_id: str, dir_path: str, crawl_interval: int = 1):
        ...
        for id in note_id_list:
            ...
            try:
                ...
                note = self.save_note_by_id(id, dir_path+"/"+nickName)
                SQL_UTILS.insert_data( id, json.dumps(note), SQL_UTILS.LOCAL)
            except Exception as e:
                print(e)
    def save_note_by_id(self, note_id: str, dir_path: str, note=None):
        if note is None:
            note = self.get_note_by_id(note_id)
        ...
        if note["type"] == NoteType.VIDEO.value:
            video_url = get_video_url_from_note(note)
            video_filename = os.path.join(new_dir_path, f"11.mp4")
            download_file(video_url, video_filename)
            content += f"\n![](11.mp4)"
        else:
            img_urls = get_imgs_url_from_note(note)
